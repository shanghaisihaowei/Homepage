  ...
  <div class="row">
      ...
      <q-card-section class="q-pa-md">
        ...
        <div class="row fir_amount">
          ...
          <q-input
            ...
            :rules="[val => val && val >= 100 || this.amount_small + '100',val => val && this.withdrawal_amount * 1 >= val * 1 || this.beyond]"
            outlined
        ...
        <div v-if="area === 'China'" class="w_notice">
          {{ $t('community.withdraw.notice3') }}
          <br>
          {{ $t('community.withdraw.notice4') }}
        </div>
        <div v-if="area !== 'China'" class="w_notice">
    ...
    <q-card class="col-12 shadow-0" style="margin-top: 30px">
      ...
      <q-card-section horizontal class="q-pa-md row">
        ...
        <!--        切换账户-->
        <q-btn-dropdown v-if="area !== 'China'" class="col-2 toggle_btn" unelevated :label=chose_amount>
          <q-list>
            <q-item v-if="area !== 'China'" v-for="item in amount" clickable v-close-popup @click="chose_accounts(item.account_number)">
              <q-item-section>
                <q-item-label>{{ item.account_number }}</q-item-label>
              </q-item-section>
            </q-item>
          </q-list>
        </q-btn-dropdown>
      ...
      <q-card-section class="q-pa-md">
        <div class="acc_info">
          ...
          &nbsp;&nbsp;&nbsp;
         <span v-if="area !== 'China'">PayPal</span>
          <span v-if="area === 'China'">{{ $t('community.withdraw.alipay') }}</span>
        </div>
        <div class="acc_info">
    ...
    <q-dialog v-model="notice" persistent full-width>
...
import {getauth, post, postauth} from "boot/axios";
...
export default defineComponent({
  ...
  methods: {
    ...
    getWalletDetail() {
      var _this = this
      getauth('pay/api/v1/d_wallet/').then((res) => {
    ...
    account_list() {
      ...
      if (_this.$q.cookies.has('token')) {
        if (_this.area !== 'China') {
          getauth('/pay/api/v1/usd_account/?').then(res => {
            _this.amount = res
            if (res.length > 0) {
              res.forEach((item, index) => {
                if (item.is_default) {
                  _this.amount_num = item.paypal_account
                }
              })
            } else {
            }
    ...
    submit() {
      ...
      if (_this.withdrawal_amount * 1 >= _this.amount_sub * 1
        && _this.amount_sub * 1 >= 100
        && _this.amount_num !== ''
      ) {
        var msg = {}
        msg.balance = _this.amount_sub
        msg.paypal_account = _this.amount_num
        if (_this.$q.cookies.get('area') === 'China') {
          msg.withdrawal_type = 'alipay'
        } else {
          msg.withdrawal_type = 'paypal'
        }
        postauth('pay/api/v1/d_cash/',msg).then(res => {
          console.log(res)
          //可以提交
          _this.$q.notify({
